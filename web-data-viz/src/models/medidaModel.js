...
function buscarUltimasMedidas(idUsuario) {
    ...
    var instrucaoSql = `SELECT LEFT(CAST((COUNT(*) * 100 / (SELECT COUNT(*) FROM resposta WHERE fkQuestao = 1 and fkUsuario = ${idUsuario})) AS CHAR), 2) AS percentagem_respostasUsuario_certas1
    FROM resposta WHERE correta = 'sim' AND fkQuestao = 1 and fkUsuario = ${idUsuario};`;
...
function buscarUltimasMedidas2(idUsuario) {
    ...
    var instrucaoSql = `SELECT LEFT(CAST((COUNT(*) * 100 / (SELECT COUNT(*) FROM resposta WHERE fkQuestao = 2 and fkUsuario = ${idUsuario})) AS CHAR), 2) AS percentagem_respostaUsuarios_certas2 FROM resposta WHERE correta = 'sim' AND fkQuestao = 2 and fkUsuario = ${idUsuario};`;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
...
function buscarUltimasMedidas3(idUsuario) {
    console.log("ACESSEI O MEDIDA MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function buscarUltimasMedidas2(): ")
    var instrucaoSql = `SELECT LEFT(CAST((COUNT(*) * 100 / (SELECT COUNT(*) FROM resposta WHERE fkQuestao = 1 and fkUsuario = ${idUsuario})) AS CHAR), 2) AS percentagem_respostasUsuario_erradas1 FROM resposta WHERE correta = 'nao' AND fkQuestao = 1 and fkUsuario = ${idUsuario};`;
...
function buscarUltimasMedidas4(idUsuario) {
    console.log("ACESSEI O MEDIDA MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function buscarUltimasMedidas2(): ")
    var instrucaoSql = `SELECT LEFT(CAST((COUNT(*) * 100 / (SELECT COUNT(*) FROM resposta WHERE fkQuestao = 2 and fkUsuario = ${idUsuario})) AS CHAR), 2) AS percentagem_respostasUsuario_erradas2 FROM resposta WHERE correta = 'nao' AND fkQuestao = 2 and fkUsuario = ${idUsuario};`;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
